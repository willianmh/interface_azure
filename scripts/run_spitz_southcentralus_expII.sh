...
for (( i = 0; i < $NUMBER_INSTANCES + 1 ; i++ )); do
    createMachines $i ${ADMINPASSWORD} ${PASSMOUNT} ${DISKURL}collective ${USERNAME} &
    sleep 10
done
wait
...
sudo mkdir -p /home/username/mymountpoint/$FILESHARE

cp -r ../cepetro-codes /home/username/mymountpoint/${FILESHARE}
cp -r ../data /home/username/mymountpoint/${FILESHARE}

sudo rm -rf /home/username/mymountpoint/${FILESHARE}/cepetro-codes/run/spitz-crs

ssh ${SSH_ADDR} << EOF
		set -x
		# sudo rm -rf cepetro-codes/.git
		# sudo cp -r cepetro-codes mymountpoint
    #
    #
		# sudo mkdir -p /home/username/mymountpoint/data
		# sudo cp /home/username/${SPITZ_FILE} /home/username/mymountpoint/data/
...
ssh ${SSH_ADDR} ./build-spitz.sh ${NUMBER_INSTANCES} ${FILESHARE}

scp scripts/benchmarks/run-spitz.sh ${SSH_ADDR}:
ssh ${SSH_ADDR} ./run-spitz.sh ${NUMBER_INSTANCES} ${NUMBER_PROCESSORS} ${VM_SIZE} ${FILESHARE}
...
cp -r /home/username/mymountpoint/${FILESHARE}/data/${FILE_TO_PROCESS}/crs/${VM_SIZE}/* /home/username/results/${VM_SIZE}/

mkdir -p /home/username/mymountpoint/results
mkdir -p /home/username/mymountpoint/results/${VM_SIZE}
cp -r /home/username/mymountpoint/${FILESHARE}/data/${FILE_TO_PROCESS}/crs/${VM_SIZE}/* /home/username/mymountpoint/results/${VM_SIZE}/
