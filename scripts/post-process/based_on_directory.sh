...
do
	...
	for j in $(ls $RESULTS_DIRECTORY)
	do
			# FOLDER=$(sed 's/,.*//' <<<$j )
			FOLDER=$j
			# echo $FOLDER

			if [ -d "$RESULTS_DIRECTORY/$FOLDER" ]
			then
				echo $FOLDER | tr '\n' ',' >> $RESULT_FILE
				...
				then
					...
					PRICE=$(cat ../../machines/all_vms_${LOCATION}_sort | grep -w $FOLDER | awk -F "\"*,\"*" '{print $5}')
					PRICE=$((${i} * ${PRICE}))
...
sed -i 's/,$//' $RESULT_FILE
(head -n 1 $RESULT_FILE && tail -n +2 $RESULT_FILE | sort -k1 -n -t, ) > $RESULTS_DIRECTORY/result_sort.csv
#
# for i in `seq 0 5`; do
# 	echo " $((2**${i})) cores" | tr '\n' ',' >> result.csv
# done
